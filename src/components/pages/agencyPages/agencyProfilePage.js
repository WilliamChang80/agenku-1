...

      {
        <div>
          ...
          {state?.thisAgencyData.services.length === 0 ? (
            ...
            <div>
              <span>
                You have {state?.thisAgencyData.services.length} services
              </span>
              <div className={"d-flex flex-wrap"}>
                {state?.thisAgencyData.services.map(item => {
                  return (
                    <div className={"mx-3 shadow-lg p-3"}>
                      {Object.entries(item).map(([key, isi]) => {
                        ...
                        return (
                          <div key={key}>
                            <div className={"my-3"}>
                              service {key} :{" "}
                              {typeof isi === "object" ? isi.name : isi}
                            </div>
                          </div>
                        )
                      })}
            ...
            <div className={" pb-5 align-items-center d-flex"}>
              ...
              <div className={"mx-3"}>
                <Link
                  to={"/agency/chat"}
                  className={"btn btn-primary"}
                  state={{ thisAgencyId: state?.thisAgencyData.id }}
                >
                  ini chat untuk liat chat
                </Link>
              </div>
            </div>
          )}
